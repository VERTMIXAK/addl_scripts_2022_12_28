...
ii=25;jj=10;
fig(1);clf;plot(N20_w(32,:,jj,ii));hold on;plot(N2_w_ROMS(32,:,jj,ii),'r');
ii=30;jj=90;plot(N20_w(32,:,jj,ii));hold on;plot(N2_w_ROMS(32,:,jj,ii),'r')
title('N2 within ROMS vs N2 calculated from T and S for a couple X,Y')
...
tt=32;ii=80;jj=60;nsmoo=3;
dum=sq(N2_w_ROMS(tt,:,jj,ii));
fig(2);clf;plot(dum);hold on;plot(smooth(dum,nsmoo),'r');
title(['N2(tt=',num2str(tt),',jj=',num2str(jj),',ii=',num2str(ii),') versus a smoothed version'])
...
% ii=80;jj=60;  %also good
nVel = 3;
nRho = 3;

PatXY  = zeros(nVel,nt,nz);
WatXY  = zeros(nRho,nt,nz);

['Evaluating all N2(t) at (jj=',num2str(jj),',ii=',num2str(ii),')']
...
%         rho_r = RHO(tt,:,jj,ii)';
        ...
        n2_w = N2_w_smoo(tt,:,jj,ii)';%n2_w(n2_w<1e-8)=1e-8;
...
done(['Evaluating all N2(t) at (jj=',num2str(jj),',ii=',num2str(ii),')'])

%% Simplex method: use t=0 modes at X,Y for all t. Keep ii,jj as above
...
%     n2_r = N2_r(tt,:,jj,ii)'; % n2_r(n2_r<1e-8)=1e-8;
    ...
    for nn=1:nVel
        ...
        num = dot(sq(PatXY(nn,1,:)) .* dz,u_r);
        denom = sum( sq(PatXY(nn,1,:)).^2 .* dz  );        
        eigUsimplex(nn,tt) = num/denom;
        
        % case 2: find the eigencoefficients from the updated wmodes
        num = dot(sq(PatXY(nn,tt,:)) .* dz,u_r);
        denom = sum( sq(PatXY(nn,tt,:)).^2 .* dz  );        
        eigUideal(nn,tt) = num/denom;
    end; %nn
    
    for nn=1:nRho
        % case 1: find the eigencoefficients from the t=0 wmodes
        num = dot(sq(WatXY(nn,1,:)) .* dz,rho_r);
        denom = sum( n2_r .* sq(WatXY(nn,1,:)).^2 .* dz  );
        eigRsimplex(nn,tt) = num/denom;
        
        % case 2: find the eigencoefficients from the updated wmodes
        num = dot(sq(WatXY(nn,tt,:)) .* dz,rho_r);
        denom = sum( n2_r .* sq(WatXY(nn,tt,:)).^2 .* dz  );
...
subplot(3,1,1);plot(z_r,sq(PatXY(1,:,:))');ylabel('pmode 1');axis([0 max(z_r) -1.1 1.1]);xlabel('z');hold on;plot(z_r,sq(PatXY(1,1,:)),'r','LineWidth',3)
subplot(3,1,2);plot(z_r,sq(PatXY(2,:,:))');ylabel('pmode 2');axis([0 max(z_r) -1.1 1.1]);xlabel('z');hold on;plot(z_r,sq(PatXY(2,1,:)),'r','LineWidth',3)
subplot(3,1,3);plot(z_r,sq(PatXY(3,:,:))');ylabel('pmode 3');axis([0 max(z_r) -1.1 1.1]);xlabel('z');hold on;plot(z_r,sq(PatXY(3,1,:)),'r','LineWidth',3)
...
suptitle('U eigencoefficients. Ideal (red) vs Simplex (blue)')
fig(8);clf;suptitle('Wmodes at X,Y -  31 days')
subplot(3,1,1);plot(z_r,sq(WatXY(1,:,:))');ylabel('wmode 1');axis([0 max(z_r)  0   1.1]);xlabel('z');hold on;plot(z_r,sq(WatXY(1,1,:)),'r','LineWidth',3)
subplot(3,1,2);plot(z_r,sq(WatXY(2,:,:))');ylabel('wmode 2');axis([0 max(z_r) -1.1 1.1]);xlabel('z');hold on;plot(z_r,sq(WatXY(2,1,:)),'r','LineWidth',3)
subplot(3,1,3);plot(z_r,sq(WatXY(3,:,:))');ylabel('wmode 3');axis([0 max(z_r) -1.1 1.1]);xlabel('z');hold on;plot(z_r,sq(WatXY(3,1,:)),'r','LineWidth',3)
...
%% Best-N2 method: Keep ii,jj as above

% The idea is this: there are many sites which have very nearly the same
% ocean depth as my test point, H(JJ,II). It is probable that one of these
...
myXYinPool=intersect(find(jj == jPool),find(ii == iPool))
...
%   N2_r_ofTatXY    N2 on rho grid, at point X,Y for all values of T
%   N2_r_Pool       N2 on rho grid at t=0, for a set of points with H close to H(X,Y)

N2_r_ofTatXY = sq(N2_r_smoo(:,:,jj,ii));
% N2_r_ofTatXY(N2_r_ofTatXY<1e-8)=1e-8;
...
% 
% for tt=1:nt
%     N2_r_ofTatXY(tt,:) = N2_r_ofTatXY(tt,:) / norm(N2_r_ofTatXY(tt,:) .* dz');
% end;%tt
% 
% for mm=1:length(jPool)
...
fig(10);clf;plot(N2_r_ofTatXY(2:end-1,:)');hold on;plot(N2_r_ofTatXY(1,:),'b','LineWidth',3);plot(N2_r_ofTatXY(nt,:),'r','LineWidth',3);
...
fig(11);clf;plot(N2_r_Pool(:,:)');hold on;plot(N2_r_ofTatXY(end,:),'r','LineWidth',3);
title('N2(Tfinal,Y,X) and the pool of relevant N2 at t=0')

% fig(12);clf;plot(N2_r_ofTatXY(1,:),'b');hold on;plot(N2_r_Pool(myXYinPool,:),'r');
...
% fig(11);clf;plot(sq(PatXY(:,1,:))','b');hold on;plot(sq(PPool(:,myXYinPool,:))','r');
% title('make sure the modes look OK')
% 
% fig(12);clf;plot(sq(WatXY(:,1,:))','b');hold on;plot(sq(WPool(:,myXYinPool,:))','r');
% title('make sure the modes look OK')


%% Ok, now do a little reverse engineering. 
%   I have all the pmodes calculated so I can figure out which I like the
%   best then come up with a rule that choose the N2 from my pool that
%   gives me those modes.

myT = 16;
myList = zeros(length(jPool),1);
myN2ofT = N2_r_ofTatXY(myT,:)';
for mm=1:length(jPool)
    for rr=1:nRho
        myList(mm) = myList(mm) + norm( sq(WPool(rr,mm,:) - WatXY(rr,myT,:)) );
    end;%rr
    for vv=1:nVel
        myList(mm) = myList(mm) + norm( sq(PPool(vv,mm,:) - PatXY(vv,myT,:)) );
    end;%vv
end;%mm

myBest=find(min(myList)==myList)
fig(20);clf;suptitle(['Scheme 1 works. myBest = ',num2str(myBest),10])
% plot(N2_r_ofTatXY(myT,:),'b');hold on;plot(N2_r_ofTatXY(1,:),'r');plot(N2_r_Pool(myBest,:),'g');
subplot(3,2,1);plot(N2_r_ofTatXY(myT,:),'b');hold on;plot(N2_r_ofTatXY(1,:),'r');
    plot(N2_r_Pool(myBest,:),'g');title('unnormalized');ylabel('N2')
subplot(3,2,3);plot(N2_r_ofTatXY(myT,:)/max(N2_r_ofTatXY(myT,:)),'b');hold on;plot(N2_r_ofTatXY(1,:)/max(N2_r_ofTatXY(1,:)),'r');
    plot(N2_r_Pool(myBest,:)/max(N2_r_Pool(myBest,:)),'g');title('normalized to max value');ylabel('N2')
subplot(3,2,5);plot(N2_r_ofTatXY(myT,:)/norm(N2_r_ofTatXY(myT,:) .* dz'),'b');hold on;plot(N2_r_ofTatXY(1,:)/norm(N2_r_ofTatXY(1,:) .* dz'),'r');
    plot(N2_r_Pool(myBest,:)/norm(N2_r_Pool(myBest,:) .* dz'),'g');title('normalized to rms integral');ylabel('N2')
subplot(3,2,2);plot(z_r,N2_r_ofTatXY(myT,:),'b');hold on;plot(z_r,N2_r_ofTatXY(1,:),'r');
    plot(z_r,N2_r_Pool(myBest,:),'g');title('unnormalized');ylabel('N2')
subplot(3,2,4);plot(z_r,N2_r_ofTatXY(myT,:)/max(N2_r_ofTatXY(myT,:)),'b');hold on;plot(z_r,N2_r_ofTatXY(1,:)/max(N2_r_ofTatXY(1,:)),'r');
    plot(z_r,N2_r_Pool(myBest,:)/max(N2_r_Pool(myBest,:)),'g');title('normalized to max value');ylabel('N2')
subplot(3,2,6);plot(z_r,N2_r_ofTatXY(myT,:)/norm(N2_r_ofTatXY(myT,:) .* dz'),'b');hold on;plot(z_r,N2_r_ofTatXY(1,:)/norm(N2_r_ofTatXY(1,:) .* dz'),'r');
...
subplot(3,2,1);plot(sq(PatXY(1,myT,:)),'b');hold on;plot(sq(PatXY(1,1,:)),'r');plot(sq(PPool(1,myBest,:)),'g');ylabel('pmode 1')
subplot(3,2,3);plot(sq(PatXY(2,myT,:)),'b');hold on;plot(sq(PatXY(2,1,:)),'r');plot(sq(PPool(2,myBest,:)),'g');ylabel('pmode 2')
subplot(3,2,5);plot(sq(PatXY(3,myT,:)),'b');hold on;plot(sq(PatXY(3,1,:)),'r');plot(sq(PPool(3,myBest,:)),'g');ylabel('pmode 3')
subplot(3,2,2);plot(sq(WatXY(1,myT,:)),'b');hold on;plot(sq(WatXY(1,1,:)),'r');plot(sq(WPool(1,myBest,:)),'g');ylabel('wmode 1')
subplot(3,2,4);plot(sq(WatXY(2,myT,:)),'b');hold on;plot(sq(WatXY(2,1,:)),'r');plot(sq(WPool(2,myBest,:)),'g');ylabel('wmode 2')
subplot(3,2,6);plot(sq(WatXY(3,myT,:)),'b');hold on;plot(sq(WatXY(3,1,:)),'r');plot(sq(WPool(3,myBest,:)),'g');ylabel('wmode 3')

% This looks pretty useful - bottom right
fig(22);clf;suptitle(['Scheme 1 works. myBest = ',num2str(myBest),10])
% plot(N2_r_ofTatXY(myT,:),'b');hold on;plot(N2_r_ofTatXY(1,:),'r');plot(N2_r_Pool(myBest,:),'g');
subplot(3,2,1);plot(N2_r_ofTatXY(myT,:).*dz','b');hold on;plot(N2_r_ofTatXY(1,:).*dz','r');
    plot(N2_r_Pool(myBest,:).*dz','g');title('unnormalized');ylabel('N2 dz')
subplot(3,2,3);plot((N2_r_ofTatXY(myT,:)/max(N2_r_ofTatXY(myT,:))).*dz','b');hold on;plot((N2_r_ofTatXY(1,:)/max(N2_r_ofTatXY(1,:))).*dz','r');
    plot((N2_r_Pool(myBest,:)/max(N2_r_Pool(myBest,:))).*dz','g');title('normalized to max value');ylabel('N2 dz')
subplot(3,2,5);plot((N2_r_ofTatXY(myT,:)/norm(N2_r_ofTatXY(myT,:) .* dz')).*dz','b');hold on;plot((N2_r_ofTatXY(1,:)/norm(N2_r_ofTatXY(1,:) .* dz')).*dz','r');
    plot((N2_r_Pool(myBest,:)/norm(N2_r_Pool(myBest,:) .* dz')).*dz','g');title('normalized to rms integral');ylabel('N2 dz')
subplot(3,2,2);plot(z_r,N2_r_ofTatXY(myT,:).*dz','b');hold on;plot(z_r,N2_r_ofTatXY(1,:).*dz','r');
    plot(z_r,N2_r_Pool(myBest,:).*dz','g');title('unnormalized');ylabel('N2 dz')
subplot(3,2,4);plot(z_r,(N2_r_ofTatXY(myT,:)/max(N2_r_ofTatXY(myT,:))).*dz','b');hold on;plot(z_r,(N2_r_ofTatXY(1,:)/max(N2_r_ofTatXY(1,:))).*dz','r');
    plot(z_r,(N2_r_Pool(myBest,:)/max(N2_r_Pool(myBest,:))).*dz','g');title('normalized to max value');ylabel('N2 dz')
subplot(3,2,6);plot(z_r,(N2_r_ofTatXY(myT,:)/norm(N2_r_ofTatXY(myT,:) .* dz')).*dz','b');hold on;plot(z_r,(N2_r_ofTatXY(1,:)/norm(N2_r_ofTatXY(1,:) .* dz')).*dz','r');
...
fig(25);clf;
plot(z_r,(N2_r_ofTatXY(myT,:)/norm(N2_r_ofTatXY(myT,:) .* dz')).*dz','b');hold on;plot(z_r,(N2_r_ofTatXY(1,:)/norm(N2_r_ofTatXY(1,:) .* dz')).*dz','r');
...
% % for mm=1:length(jPool)
% %     for rr=1:nRho
% %         myList(mm) = myList(mm) + norm( (sq(WPool(rr,mm,:) - WatXY(rr,myT,:))) .* dz );
% %     end;%rr
% %     for vv=1:nVel
% %         myList(mm) = myList(mm) + norm( (sq(PPool(vv,mm,:) - PatXY(vv,myT,:))) .* dz );
% %     end;%vv
% % end;%mm
% % 
% % myBest=find(min(myList)==myList)
...
% % subplot(3,2,1);plot(sq(PatXY(1,myT,:)),'b');hold on;plot(sq(PatXY(1,1,:)),'r');plot(sq(PPool(1,myBest,:)),'g');ylabel('pmode1')
% % subplot(3,2,3);plot(sq(PatXY(2,myT,:)),'b');hold on;plot(sq(PatXY(2,1,:)),'r');plot(sq(PPool(2,myBest,:)),'g');ylabel('pmode2')
% % subplot(3,2,5);plot(sq(PatXY(3,myT,:)),'b');hold on;plot(sq(PatXY(3,1,:)),'r');plot(sq(PPool(3,myBest,:)),'g');ylabel('pmode3')
% % subplot(3,2,2);plot(sq(WatXY(1,myT,:)),'b');hold on;plot(sq(WatXY(1,1,:)),'r');plot(sq(WPool(1,myBest,:)),'g');ylabel('wmode1')
% % subplot(3,2,4);plot(sq(WatXY(2,myT,:)),'b');hold on;plot(sq(WatXY(2,1,:)),'r');plot(sq(WPool(2,myBest,:)),'g');ylabel('wmode2')
% % subplot(3,2,6);plot(sq(WatXY(3,myT,:)),'b');hold on;plot(sq(WatXY(3,1,:)),'r');plot(sq(WPool(3,myBest,:)),'g');ylabel('wmode3')
...
% % % for mm=1:length(jPool)
% % %     for rr=1:nRho
% % %         myList(mm) = myList(mm) + dot( sq(WPool(rr,mm,:)), sq(WatXY(rr,myT,:)) );
% % %     end;%rr
% % %     for vv=1:nVel
% % %         myList(mm) = myList(mm) + dot( sq(PPool(vv,mm,:)), sq(PatXY(vv,myT,:)) );
% % %     end;%vv
% % % end;%mm
% % % 
...
% % % subplot(3,2,1);plot(sq(PatXY(1,myT,:)),'b');hold on;plot(sq(PatXY(1,1,:)),'r');plot(sq(PPool(1,myBest,:)),'g')
% % % subplot(3,2,3);plot(sq(PatXY(2,myT,:)),'b');hold on;plot(sq(PatXY(2,1,:)),'r');plot(sq(PPool(2,myBest,:)),'g')
% % % subplot(3,2,5);plot(sq(PatXY(3,myT,:)),'b');hold on;plot(sq(PatXY(3,1,:)),'r');plot(sq(PPool(3,myBest,:)),'g')
% % % subplot(3,2,2);plot(sq(WatXY(1,myT,:)),'b');hold on;plot(sq(WatXY(1,1,:)),'r');plot(sq(WPool(1,myBest,:)),'g')
% % % subplot(3,2,4);plot(sq(WatXY(2,myT,:)),'b');hold on;plot(sq(WatXY(2,1,:)),'r');plot(sq(WPool(2,myBest,:)),'g')
% % % subplot(3,2,6);plot(sq(WatXY(3,myT,:)),'b');hold on;plot(sq(WatXY(3,1,:)),'r');plot(sq(WPool(3,myBest,:)),'g')
...
%% Ok, now I want to pick some N2_r_ofTatXY and select the element of N2_r_Pool
...
% % % myBest=find(min(myList)==myList)
% % % 
% % % fig(13);clf;plot(N2_r_ofTatXY(myT,:),'b');hold on;plot(N2_r_Pool(myBest,:),'r')
% % % 
% % % fig(15);clf;plot(sq(PatXY(:,myT,:))','b');hold on;plot(sq(PPool(:,myBest,:))','r');
% % % title('make sure the modes look OK')
% % % 
% % % fig(16);clf;plot(sq(WatXY(:,myT,:))','b');hold on;plot(sq(WPool(:,myBest,:))','r');
% % % title('make sure the modes look OK')



% fig(15);clf;plot(sq(WatXY(:,myT,:))','b');hold on;plot(sq(WatXY(:,1,:))','r');plot(sq(WPool(:,myBest,:))','g')
