...
% JGP S2 only at this point
consts = {'s2'};%{'m2','s2','n2','k2','o1','p1','k1'}
%
% get model amplitude

model.exp1 = ['/import/c/w/jpender/roms-kate_svn/'];
model.exp2 = 'TS_0.125/Experiments/TS_0.125_2014_001_TidesS2_30days/';figno=1;
...
eval(['outfile =  ''','./',model.file_prefix,num2str(lon0),'_',num2str(lon1),'_',num2str(lat0),'_',num2str(lat1),'_',num2str(hskip),'_S2.mat'''])

if ~exist(outfile)
[tpxo.lon,tpxo.lat,tpxo.amp,tpxo.pha]=tmd_get_coeff(mfile,'z','S2');done('loading tpxo')
...
figure(1);clf;
subplot(2,2,1)
imagesc(model.lon,model.lat,model.amp);axis xy;caxis([0,.3]);rect;hold on
               contour(model.lon,model.lat,model.amp,[0:.05:1],'LineColor','Black','Showtext','on')
               title('ROMS - S2 amplitude');colorbar
...
imagesc(model.lon,model.lat,model.pha);axis xy;caxis([0,360]);rect;hold on
               ...
               title('ROMS - S2 phase');colorbar

subplot(2,2,3)
idxs = find(tpxo.lon>=model.lon(1)&tpxo.lon<=model.lon(end));
jdxs = find(tpxo.lat>=model.lat(1)&tpxo.lat<=model.lat(end));
imagesc(tpxo.lon(idxs),tpxo.lat(jdxs),tpxo.amp(jdxs,idxs));axis xy;caxis([0,.3]);rect;hold on
               contour(tpxo.lon(idxs),tpxo.lat(jdxs),tpxo.amp(jdxs,idxs),[0:.05:1],'LineColor','Black','Showtext','on')
               title('TPXO, S2 amplitude');colorbar
...
% 
% forcingFile = '/import/c/w/jpender/ROMS/realWork/EIO_32Grid/Tides_S2only_Sindhu/EIO_32_tides_otps_topo30.nc';
...
%                title('forcing amplitude, S2 forcing');colorbar
