...
% JGP K1 only at this point
consts = {'k1'};%{'m2','s2','n2','k2','o1','p1','k1'}
%
% get model amplitude

model.exp1 = ['/import/c/w/jpender/roms-kate_svn/'];
model.exp2 = 'TS_0.125/Experiments/TS_0.125_2014_001_TidesK1_30days/';figno=1;

model.file_prefix = 'TS_';
model.file_suffix = '_2014';
% disp(['!head -500 ',model.exp1,model.exp2,'log |grep "Input Grid"  | awk ''{print $4}'' >! gfile '])
% eval(['!head -500 ',model.exp1,model.exp2,'log |grep "Input Grid"  | awk ''{print $4}'' >! gfile '])
...
if ~exist( ['./',model.file_prefix,'his2',model.file_suffix,'_zeta.nc'])
    ...
    eval(['!ncrcat -v zeta -O ','../netcdfOutput/',model.file_prefix,'his2_2014_0*.nc ','./',model.file_prefix,'his2',model.file_suffix,'_zeta.nc'])
end

eval(['outfile =  ''','./',model.file_prefix,num2str(lon0),'_',num2str(lon1),'_',num2str(lat0),'_',num2str(lat1),'_',num2str(hskip),'_K1.mat'''])

if ~exist(outfile)
[tpxo.lon,tpxo.lat,tpxo.amp,tpxo.pha]=tmd_get_coeff(mfile,'z','K1');done('loading tpxo')
...
% figure(figno);clf;
% subplot(1,2,1);hformat(12);imagesc(model.lon,model.lat,model.amp);axis xy;caxis([0,.5]);rect;hold on
%                contour(model.lon,model.lat,model.pha-47,[0:15:345],'k');colorbar;
%                title('MODEL, M2 forcing');
% idxs = find(tpxo.lon>=model.lon(1)&tpxo.lon<=model.lon(end));
% jdxs = find(tpxo.lat>=model.lat(1)&tpxo.lat<=model.lat(end));
% subplot(1,2,2);hformat(12);imagesc(tpxo.lon(idxs),tpxo.lat(jdxs),tpxo.amp(jdxs,idxs));axis xy;caxis([0,.5]);rect;title('TPXO, M2 forcing');hol$
%                contour(tpxo.lon(idxs),tpxo.lat(jdxs),tpxo.pha(jdxs,idxs),[0:15:345],'k');colorbar

figure(1);clf;
subplot(2,2,1)
imagesc(model.lon,model.lat,model.amp);axis xy;caxis([0,.25]);rect;hold on
               contour(model.lon,model.lat,model.amp,[0:.05:1],'LineColor','Black','Showtext','on')
               title('ROMS - K1 amplitude');colorbar
...
imagesc(model.lon,model.lat,model.pha);axis xy;caxis([0,360]);rect;hold on
               ...
               title('ROMS - K1 phase');colorbar

subplot(2,2,3)
idxs = find(tpxo.lon>=model.lon(1)&tpxo.lon<=model.lon(end));
jdxs = find(tpxo.lat>=model.lat(1)&tpxo.lat<=model.lat(end));
imagesc(tpxo.lon(idxs),tpxo.lat(jdxs),tpxo.amp(jdxs,idxs));axis xy;caxis([0,.25]);rect;hold on
               contour(tpxo.lon(idxs),tpxo.lat(jdxs),tpxo.amp(jdxs,idxs),[0:.05:1],'LineColor','Black','Showtext','on')
               title('TPXO, K1 amplitude');colorbar
...
% 
% forcingFile = '/import/c/w/jpender/ROMS/realWork/EIO_32Grid/Tides_K1only_Sindhu/EIO_32_tides_otps_topo30.nc';
...
%                title('forcing amplitude, K1 forcing');colorbar
