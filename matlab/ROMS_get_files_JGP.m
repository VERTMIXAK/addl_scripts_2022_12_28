...
%roms.analysis_path = ['/import/c/w/',roms.analysis_loc,'/roms-kate_svn/'];
%% 
%keyboard
%%
...
% roms.base2     = ['/import/c/w/',roms.analysis_loc,'/roms-kate_svn/'];
% roms.analysis_path     = [roms.base2,'/',roms.expbase,'/Experiments/',roms.exp1,'/Analysis/'];
...
if isfield(roms.files,'his_daily_files');hfile = roms.files.his_daily_files{1};else;hfile = roms.files.his_hourly_files{1};end
    ...
    if roms.lat0 < min(roms.grd.lat_rho(:)) || roms.lat1 > max(roms.grd.lat_rho(:)) || roms.lon0 < min(roms.grd.lon_rho(:)) || roms.lon1 > max(roms.grd.lon_rho(:))
        'the lat/lon limits are screwed up'
        keyboard
    end
