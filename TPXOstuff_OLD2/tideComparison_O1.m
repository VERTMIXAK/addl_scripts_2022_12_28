...
tpxomodel='~/archROMS/OTIS_DATA/Model_tpxo7.2';
...
% PALAU_120B footprint - this is to size the TPXO stuff
...
imagesc(model.lon,model.lat,model.amp_zeta);axis xy;
caxis(ampLim);
rect;hold on
...
subplot(2,2,2)
imagesc(model.lon,model.lat,model.pha_zeta);axis xy;
caxis(phaLIm);
rect;hold on
...
imagesc(model.lon,model.lat, model.amp_ubar);axis xy;
caxis(ampLim);
rect;hold on
...
subplot(2,2,2)
imagesc(model.lon,model.lat,model.pha_ubar);axis xy;
caxis(phaLim);
rect;hold on
...
imagesc(model.lon,model.lat,  model.amp_vbar);axis xy;
caxis(ampLim);
rect;hold on
...
subplot(2,2,2)
imagesc(model.lon,model.lat,model.pha_vbar);axis xy;
caxis(phaLim);
rect;hold on
