...
lonGrid=subGrid.lonUgrid;
latGrid=subGrid.latUgrid;
fileList=subGrid.fileList;

lat_min=subGrid.lat_u_min;
lat_max=subGrid.lat_u_max;
lon_min=subGrid.lon_u_min;
lon_max=subGrid.lon_u_max;
...
outFile = strcat('../netcdf_regrid/U',range,'.nc')
unix('mkdir ../netcdf_regrid');

nc_create_empty(outFile,nc_64bit_offset_mode);

% Dimension section
nc_add_dimension(outFile,'lon_u',length(lonGrid));
nc_add_dimension(outFile,'lat_u',length(latGrid));
...
dum.Name = 'u';
dum.Nctype = 'float';
dum.Dimension = {'ocean_time','z','lat_u','lon_u'};
dum.Attribute = struct('Name',{'long_name','units','time','coordinates','field'},'Value',{'u-momentum component','meter second-1','ocean_time','lon_u lat_u z ocean_time','v-velocity, scalar, series'});
nc_addvar(outFile,dum);
...
% Fill in grid data
nc_varput(outFile,'lat_u',latGrid);
nc_varput(outFile,'lon_u',lonGrid);
...
for ii = [1:length(fileList)]
    ...
    VARxieta = squeeze( nc_varget(hisFile,'u',   [0 0 0 0],[1 -1 -1 -1]) );
    VARnew = roms_zslice_var(VARxieta,zGrid,ROMSout,zsliceStuff);

    nc_varput(outFile,'u',   VARnew(1,:,lat_min:lat_max,lon_min:lon_max),[ii-1 0 0 0],[1 length(zGrid) length(latGrid) length(lonGrid) ]);disp(sprintf('done with z%i',ii))
