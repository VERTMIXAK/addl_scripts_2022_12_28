...
outFile = strcat('../netcdf_regrid/T',range,'.nc')
...
dum.Attribute = struct('Name',{'long_name','units','time','coordinates','field'},'Value',{'temperature','Celsius','ocean_time','lon_rho lat_rho z ocean_time','potential temperature, scalar, series'});
nc_addvar(outFile,dum);
...
for ii = [1:length(fileList)]
    ...
    VARxieta = squeeze( nc_varget(hisFile,'temp',   [0 0 0 0],[1 -1 -1 -1]) );
    VARnew = roms_zslice_var(VARxieta,zGrid,ROMSout,zsliceStuff);

    nc_varput(outFile,'temp',   VARnew(1,:,lat_min:lat_max,lon_min:lon_max),[ii-1 0 0 0],[1 length(zGrid) length(latGrid) length(lonGrid) ]);disp(sprintf('done with z%i',ii))
